...
class FoodAdapter(private val context: Context, private val foods : ArrayList<Food>,
                  private val foodViewModel: MenuViewModel): RecyclerView.Adapter<FoodAdapter.ViewHolder>() {
    inner class ViewHolder(private val binding : RvCardMakananBinding): RecyclerView.ViewHolder(binding.root) {
        fun bind(food: Food){
            ...
            when(food.image){
                1 -> binding.cardMakananIvImage.setImageResource(R.drawable.ayamwoku)

                2 -> binding.cardMakananIvImage.setImageResource(R.drawable.natto)

                3 -> binding.cardMakananIvImage.setImageResource(R.drawable.pizza)
